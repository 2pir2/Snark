...
//this function read the input neural network layer size from layer_sizes.json
type layer_sizes_dict struct {
	LayerSizes []int `json:"layer_sizes"`
}


//this part crate a PointPolytopeList field that holds an array of codept object from specified json
type code_pt_dict struct {
	PointPolytopeList []codept `json:"point_polytope_list"`
}
//this is the struct that reads the point polytope list json
type codept struct {
	Code string    `json:"Code"`
	Pt   []float64 `json:"Pt"`
}

//from json file initial polytope. Need to figure out what does activation code do. 
type code struct {
	ActivationCode string `json:"activation_code"`
}

//from per_poly_list_with_pt. Noraml may means distance. deicision means boundry, config code need to be figured out
type facetpt struct {
...
//define general structure of polytope
type polytope_general struct {
	A        *mat.Dense    // A is a pointer to a Dense matrix(matrix mostly non-zero) from the Gonum package
    b        *mat.VecDense // b is a pointer to a VecDense vector(vector non zero) from the Gonum package
	code     string
	fullcode string
}

//from json input points file. Initial inputs
type input struct {
	InputPoint []float64 `json:"input_point"`
}

//define general structure of facet
...
//*This is the limits.json. This one is important, need to figure out what all those means.
type limits struct {
...
//this struct is never used. There is no radius json file. The read file in nnk.go is never called
type RadiusData struct {
	Radius float64 `json:"Radius"`
}

// frontend Just a gnark libaraiy that defines variables. The size of number of inputs* number of NN layers
...
//this makes sure that the item pop is the min of the heap
...
//verify if the min item is the min
...
//dotpdtf is dot product. Sub is substruct, Mul is multiply
//this function seems like to verift if val is with in range of limits
...
//this function verify if the input point is on facet.
// the facets and inputs are given
...
//seems like this is checking the linear transformation of the points and check if points are within certain range
// maybe checking the decision facet is valid
...
//maybe checking if normal facet is valid
...
//This is making sure that distanace between adversarial label and true label stored is correct
//Then it also verfies that the bias difference is correct
...
//make sure only one of rold and rnew is different
...
//checking if constructed polytope is equal to the expected polytope given weights
//this function is performing NN learning
func (circuit *verifypolytope) Define(api frontend.API) error {
	...
	//looping through layers to update weights
...
//this step is checking if the logits match
//but this is never used so I guess not important
...
//this one is ensuring the activation code at each layer is the sames
...
//performing a NN and check if the final result match
...
//scaling each index of the matrix
...
//never used. But is for dotproduct
...
//this is returning a buch of indices for a specific input points to be used to store
...
//this is to generate the project distance to the facet instead of exact distance mentioned in the paper.
...
//this is just printing out the projection and projecction square
//probably used for debug
...
//convert multiple dense matrix into arrays of Variable to used in gnark
...
//samething as above
//adde an extra layer, probably for output layer
...
//convert int double array to Variable
...
//convert polytope A to frontend version
...
//convert polytope B to frontend version
...
//convert vector to frontend version
//this is used for Ca
...
//convert vector to frontend version
//This is used for Ca_abs. This is used in the above verify projection as a denominator to calculate norm
...
//convert a PQ to frontend with scaling
...
//first round the weights into the closed integer after some scaling
//then print if they are not equal to the default
//also set the out of bound index to be zero
...
//simialr things above but with bias
...
//this returns only the rounded and padded matrix.
...
//this return only the biases matrix
...
//This called a bunch of verify function 
...
//this is to find the code point in point polytope list
...
//it will take a valid indices and return the valid indices from code
...
//this function served for the verify adversarial struc
...
//using groth16 to verify the proof
...
//groth16 verify without file input
//Everything is the same except printing result has no file path
...
//never used. 
//return average, sum, and standard deviation of an array of integers.
...
//absolute value of a vector without the first element.
...
//find the point based on if the facet is a decision facet or not
...
//There is no output, so I guess it is for debug
//It prints the difference between the first and second element of a vector after doing linear transfomratoin and added bias
...
//the part that actually perform zk proof
...
func main() {
	...
	dirPath := "E:/FairProof/FairProof-main"
	sens_vals := []int{0, 1}
	exePath, _ := os.Getwd()
